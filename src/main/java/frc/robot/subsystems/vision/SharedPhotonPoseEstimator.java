...
public class SharedPhotonPoseEstimator {

    /**
     * The SharedPhotonPoseEstimator class filters or combines readings from all the
     ...
     * {@link SharedPhotonPoseEstimator} class.
     */
    public enum PoseStrategy {
        ...
        /**
         * Use distance data from best visible tag to compute a Pose. This runs on the
         * RoboRIO in order
         * to access the robot's yaw heading, and MUST have addHeadingData called every
         * frame so heading
         * data is up-to-date.
         *
         * <p>
        ...
        /**
         * Solve a constrained version of the Perspective-n-Point problem with the
         * robot's drivebase
         * flat on the floor. This computation takes place on the RoboRIO, and typically
         * takes not more
         * than 2ms. See {@link SharedPhotonPoseEstimator.ConstrainedSolvepnpParams} and
         * {@link
         * org.photonvision.jni.ConstrainedSolvepnpJni} for details and tuning handles
         * this strategy
         * exposes. This strategy needs addHeadingData called every frame so heading
         * data is up-to-date.
         * If Multi-Tag PNP is enabled on the coprocessor, it will be used to provide an
         * initial seed to
         * the optimization algorithm -- otherwise, the multi-tag fallback strategy will
         * be used as the
         * seed.
         */
     ...
     * {@link SharedPhotonPoseEstimator.PoseStrategy}.
     * Internally, the cost function is a sum-squared of pixel reprojection error +
     * (optionally)
     * heading error * heading scale factor.
     *
     * @param headingFree
     *            If true, heading is completely free to vary. If false, heading
     *            excursions
     *            from the provided heading measurement will be penalized
     * @param headingScaleFactor
     *            If headingFree is false, this weights the cost of changing our
     *            robot
    ...
    private static final TimeInterpolatableBuffer<Rotation2d> headingBuffer = TimeInterpolatableBuffer.createBuffer(1.0);

    /**
     * Create a new SharedPhotonPoseEstimator.
     *
     * @param fieldTags
     *            A WPILib {@link AprilTagFieldLayout} linking AprilTag IDs to
     *            Pose3d objects
     ...
     *            Coordinate System</a>. Note that setting the origin of this layout
     *            object will affect the
     *            results from this class.
     * @param strategy
     *            The strategy it should use to determine the best pose.
     * @param robotToCamera
     *            Transform3d from the center of the robot to the camera mount
     *            position (ie,
     *            robot ➔ camera) in the <a href=
     *            "https://docs.wpilib.org/en/stable/docs/software/advanced-controls/geometry/coordinate-systems.html#robot-coordinate-system">Robot
     *            Coordinate System</a>.
     */
    public SharedPhotonPoseEstimator(
     ...
     *
     * @param tagModel
     *            E.g. {@link TargetModel#kAprilTag16h5}.
     */
     ...
     *            timestamp of the robot heading data.
     * @param heading
     *            Field-relative robot heading at given timestamp. Standard WPILIB
     *            field
     *            coordinates.
     */
    public static void addHeadingData(double timestampSeconds, Rotation3d heading) {
     ...
     *            coordinates.
     */
    public static void addHeadingData(double timestampSeconds, Rotation2d heading) {
     ...
     *
     * @param robotToCamera
     *            The current transform from the center of the robot to the camera
     *            mount
     *            position
     */
    ...
    /**
     * Updates the estimated position of the robot, assuming no camera calibration
     * is required for the
     ...
     * </ul>
     *
     * Will report a warning if strategy is multi-tag-on-rio because camera
     * calibration data is not
     * provided in this overload.
     *
     * @param cameraResult
     *            The latest pipeline result from the camera
     ...
     * {@code update()}.
     * <li>No targets were found in the pipeline results.
     * <li>The strategy is CONSTRAINED_SOLVEPNP, but no constrainedPnpParams were
     * provided (use the
     * other function overload).
     * </ul>
     *
     * @param cameraMatrix
     *            Camera calibration data for multi-tag-on-rio strategy - can be
     *            empty
     *            otherwise
     * @param distCoeffs
     *            Camera calibration data for multi-tag-on-rio strategy - can be
     ...
     * {@code update()}.
     * <li>No targets were found in the pipeline results.
     * <li>The strategy is CONSTRAINED_SOLVEPNP, but the provided
     * constrainedPnpParams are empty.
     * </ul>
     *
     * @param cameraMatrix
     *            Camera calibration data for multi-tag-on-rio strategy - can be
     ...
     *            empty
     *            otherwise
     * @param constrainedPnpParams
     *            Constrained SolvePNP params, if needed.
     * @return an {@link EstimatedRobotPose} with an estimated pose, timestamp, and
     *         targets used to
     *         create the estimate.
     */
    ...
    /**
     * Internal convenience method for using a fallback strategy for update(). This
     * should only be
     ...
     *            The latest pipeline result from the camera
     * @param strategy
     *            The pose strategy to use. Can't be CONSTRAINED_SOLVEPNP.
     * @return an {@link EstimatedRobotPose} with an estimated pose, timestamp, and
     *         targets used to
     *         create the estimate.
     */
    ...
    private Optional<EstimatedRobotPose> update(
        ...
        Optional<EstimatedRobotPose> estimatedPose = switch (strategy) {
            case LOWEST_AMBIGUITY -> estimateLowestAmbiguity(cameraResult);
            case CLOSEST_TO_CAMERA_HEIGHT -> estimateClosestToCameraHeight(cameraResult);
            ...
            case AVERAGE_BEST_TARGETS -> estimateAverageBestTargetsStrategy(cameraResult);
            case MULTI_TAG_PNP_ON_COPROCESSOR -> estimateMultitagCoproc(cameraResult);
            case PNP_DISTANCE_TRIG_SOLVE -> estimatePNPDistanceTrigSolve(cameraResult);
            case CONSTRAINED_SOLVEPNP -> estimateConstrainedPNP(cameraResult, cameraMatrix, distCoeffs, constrainedPnpParams);
        };
    ...
    public Optional<EstimatedRobotPose> estimatePNPDistanceTrigSolve(PhotonPipelineResult result) {
        ...
        Translation2d camToRobotTranslation = robotToCamera.getTranslation().toTranslation2d().unaryMinus().rotateBy(headingSample);
    ...
    public Optional<EstimatedRobotPose> estimateConstrainedPNP(
    ...
    public Optional<EstimatedRobotPose> estimateMultitagCoproc(PhotonPipelineResult result) {
        ...
        return Optional.of(
            new EstimatedRobotPose(
                ...
                result.getTimestampSeconds(),
                result.getTargets(),
                result.getMultiTagResult().get().estimatedPose.ambiguity,
     ...
     *         estimation.
     */
    public Optional<EstimatedRobotPose> estimateLowestAmbiguity(PhotonPipelineResult result) {
     ...
     *         estimation.
     */
    public Optional<EstimatedRobotPose> estimateClosestToCameraHeight(PhotonPipelineResult result) {
        ...
        for (PhotonTrackedTarget target : result.targets) {
            ...
            double alternateTransformDelta = Math.abs(
                robotToCamera.getZ() - targetPosition
                    .get()
     ...
     *            pipeline result
     * @param referencePose
     *            reference pose to check vector magnitude difference against.
     * @return the estimated position of the robot in the FCS and the estimated
     *         timestamp of this
     *         estimation.
     */
    public Optional<EstimatedRobotPose> estimateClosestToReferencePose(
        ...
        if (referencePose == null) {
            DriverStation.reportError(
                "[SharedPhotonPoseEstimator] Tried to use reference pose strategy without setting the reference!",
                false);
            return Optional.empty();
        }
     ...
     *         estimation.
     */
    public Optional<EstimatedRobotPose> estimateAverageBestTargetsStrategy(PhotonPipelineResult result) {
    ...
    private void reportFiducialPoseError(int fiducialId) {
        if (!reportedErrors.contains(fiducialId)) {
            DriverStation.reportError(
                "[SharedPhotonPoseEstimator] Tried to get pose of unknown AprilTag: " + fiducialId, false);
