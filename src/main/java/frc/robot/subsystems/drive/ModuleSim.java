...
public class ModuleSim implements ModuleIO {
    ...
    @Override public void updateInputs(ModuleIOInputs inputs) {
        ...
        inputs.drive_position_radians = module_simulation.getDriveWheelFinalPosition().in(Radians);
        inputs.drive_velocity_radians_per_second = module_simulation.getDriveWheelFinalSpeed().in(RadiansPerSecond);
        inputs.drive_applied_volts = drive_applied_volts;
        inputs.drive_current_amps = Math.abs(module_simulation.getDriveMotorStatorCurrent().in(Amps));

        // Update turn inputs
        inputs.turn_connected = true;
        inputs.turn_position = module_simulation.getSteerAbsoluteFacing();
        inputs.turn_velocity_radians_per_second = module_simulation.getSteerAbsoluteEncoderSpeed().in(RadiansPerSecond);
        inputs.turn_applied_volts = turn_applied_volts;
        inputs.turn_current_amps = Math.abs(module_simulation.getSteerMotorStatorCurrent().in(Amps));

        // Update odometry inputs (50Hz because high-frequency odometry in sim doesn't
        // matter)
        inputs.odometry_timestamps = SparkUtil.getSimulationOdometryTimeStamps();
        inputs.odometry_drive_position_radians = Arrays.stream(module_simulation.getCachedDriveWheelFinalPositions())
